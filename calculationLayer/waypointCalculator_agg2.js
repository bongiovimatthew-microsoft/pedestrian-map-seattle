...
//issue-manigu-06212017 tin might not work for us
dataPoints.features.splice(200);
console.log(dataPoints);

for(i = 0; i < dataPoints.features.length; i++){
    dataPoints.features[i]['properties']['icon'] = "monument"
}

// Generate a TIN, using the "score" property as the Z value for the points 
var tin = turf.tin(dataPoints, 'score')
...
for(i = 0; i < 20; i++){
    ...
    changedCentroid = {
                       "type": "Feature",
                        ...
                        "geometry": {
                            "type": "Point",
                            "coordinates": currCentroid
                        }
                      };

    // changedCentroid.geometry.coordinates = currCentroid
    // changedCentroid.geometry.type = "point"
    // changedCentroid.properties.icon = "rocket"
    // changedCentroid.properties.latitude = currCentroid[1]
    // changedCentroid.properties.longitude = currCentroid[0]
