...
class SeattlePublicToiletsCleaner(ICleaner):  
    ...
    def GetData(self, dateRange, boundingBox):
        ...
        whereClause = "within_box(location, {0}, {1}, {2}, {3})".format(str(boundingBox[0][0]), str(boundingBox[0][1]), str(boundingBox[1][0]), str(boundingBox[1][1]))
        allFilters = []
        allFilters.append({ "$where": whereClause })
